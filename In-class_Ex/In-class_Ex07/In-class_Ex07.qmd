...
NOTE: RGDAL package is RETIRED.

NOTE: we add sp to do some functions here
...
```{r}
shan_sf <- st_read(dsn = "../../data/geospatial/myanmar", 
...
We will drop the geometry columns as the functions we will use might not be able to read it.
...
### 7.2.3 Visualizing the standardized clustering variables
...
### 7.4.1 Selecting the optimal clustering algorithm
...
## 7.5 Determining Optimal Clusters
...
## 7.6 Interpreting the dedrograms
...
## 7.7 Visually-driven hierarchical clustering analysis
...
### 7.7.1 Transforming the data frame into a matrix
...
### 7.7.2 Plotting interactive cluster heatmap using heatmaply()
...
## 7.8 Mapping the clusters formed
...
```{r}
#| eval: false
# Don't need this anymore because spdep now accepts sf features 
...
## 8.6 Visualizing the clusters in choropleth map
...
## 10.1 Visualizing individual clustering variable
...
## 10.2 Multivariate Visualization
