...
subtitle: "Geospatial Analytics Project Proposal"
format:
  html:
    code-fold: false
...
-   **Modeling Geographic Accessibility**
    ...
    -   We will map out the geographic accessibility of different regions in Singapore, taking into account all amenities, road networks, and pedestrian pathways.
...
-   **Kernel Density Estimation (KDE)**
    ...
    -   KDE will be used to analyze the density of the various amenities (e.g. supermarket, clinics, parks) across Singapore. This will help in understanding how the spread of amenities contributes to the accessibility score of an area.

# Data

## Context

For the HDB transactions, we can obtain the Resale Flat Prices directly from data.gov.sg. The data contains transactions from 1990 to present.

Source: [data.gov.sg](https://beta.data.gov.sg/collections/189/datasets/d_8b84c4ee58e3cfc0ece0d773c8ca6abc/view)

## Analysis

The data we have chosen has been compiled from multiple sources. For each amenity type, we have a list of names that have been web-scrapped or manually collected from the internet (e.g. school names from MOE). The list of names will be put through a python API ([onemapsg](https://pypi.org/project/onemapsg/)) which will take a location name as input and output its coordinates.
