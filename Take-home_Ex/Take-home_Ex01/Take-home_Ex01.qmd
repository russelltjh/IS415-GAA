...
First, let's filter the origin points that are within the Clementi area only.

```{r}
# Filter points to those within the Clementi Study Area
# This returns a logical vector suitable for direct subsetting
...
lixels <- lixelize_lines(clementi_roads, 750, mindist = 375)

# Generate Line Center Points
samples <- lines_center(lixels)

# Conduct Network Constrained KDE 
densities <- nkde(clementi_roads,
...
tm_shape(lixels) +
  ...
  # tm_shape(origin_sf_filtered) +
  # tm_dots(alpha = 0.1, size=0.01, shape=19) + 
  tm_basemap("OpenStreetMap") +
